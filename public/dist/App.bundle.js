...
function typeAhed(search) {
    ...
    searchInput.on('input', function () {
        ...
        axios.get('/api/search?q=' + this.value).then(function (res) {
            if (res.data.length) {
                var html = domPurify.sanitize(searchResultHTML(res.data));
                searchResult.innerHTML = html;
                return;
            }
            //tell them nothing found
            searchResult.innerHTML = domPurify.sanitize('<div class="search__result">No result found for ' + _this.value + ' found!</div>');
        }).catch(function (err) {
            console.error(err);
        });
    });
